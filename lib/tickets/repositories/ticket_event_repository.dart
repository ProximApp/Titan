...
class TicketEventRepository extends Repository {
  ...
  Future<List<TicketEvent>> getAllTicketEvent() async {
  ...
  Future<TicketEvent> createTicketEvent(TicketEvent tickets) async {
    return TicketEvent.fromJson(
      await create(tickets.toJson(), suffix: 'admin/events'),
    );
  }

  Future<Checkout> checkoutTicketEvent(TicketEvent tickets) async {
    return Checkout.fromJson(
      await create(
        checkoutFromShotgun(tickets).toJson(),
        suffix: 'events/${tickets.id}/checkout',
      ),
    );
  }
...
final ticketEventRepositoryProvider = Provider<TicketEventRepository>((ref) {
  ...
  return TicketEventRepository()..setToken(token);
});
