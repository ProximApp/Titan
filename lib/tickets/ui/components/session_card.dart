...
class SessionCard extends HookWidget {
  ...
  final void Function(List<SessionCreate> sessions) onChanged;
  ...
  Widget build(BuildContext context) {
    ...
    // The picker writes a locale-formatted date, which is not ISO-8601, so it
    // has to go back through the shared parser before it means anything.
    DateTime startDatetimeOf(TextEditingController date) {
      if (date.text.isEmpty) return DateTime.now();
      try {
        return DateTime.parse(processDateBackWithHourMaybe(date.text, locale));
      } catch (_) {
        return DateTime.now();
      }
    }
    ...
    void notify() => onChanged(
      entries.value
          .map(
            (e) => SessionCreate(
              name: e['label']!.text,
              startDatetime: startDatetimeOf(e['date']!),
