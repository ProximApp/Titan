...
class PermissionDetailModal extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final authorizedAccountTypes = permission?.accountTypes ?? [];
    final authorizedGroupIds = permission?.groups ?? [];
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...accountTypes.map((accountType) {
                    final isAuthorized = authorizedAccountTypes
                        .map((item) => item.name)
                        .contains(snakeToCamelCase(accountType));
                    final model = CoreAccountTypePermission(
                      permissionName: actionPermissionName,
                      accountType: AccountType.values.firstWhere(
                        (type) =>
                            type.name.contains(snakeToCamelCase(accountType)),
                      ),
                    );
                  ...
                  ...groups.map((group) {
                    ...
                    final model = CoreGroupPermission(
                      permissionName: actionPermissionName,
                      groupId: group.id,
                    );
                    return PermissionRow(
