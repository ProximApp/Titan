...
class SuperAdminsPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> openToggleDialog(String userId, String userDisplayName) async {
      ...
      final response = await repo.usersUserIdGet(userId: userId);
      final fullUser = response.body;
      if (fullUser == null || !context.mounted) return;

      final currentlySuperAdmin = fullUser.isSuperAdmin ?? false;
      ...
      await showDialog(
        ...
        builder: (dialogContext) => CustomDialogBox(
          ...
          onYes: () async {
            ...
            final ok = await repo.usersUserIdSuperAdminPatch(userId: userId);
