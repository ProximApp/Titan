...
void main() {
  group('ListListNotifier', () {
    ...
    final lists = [
      ListReturn.empty().copyWith(id: '1', type: ListType.pipo),
      ListReturn.empty().copyWith(id: '2', type: ListType.serio),
    ];
    final newList = ListReturn.empty().copyWith(id: '3');
    ...
    setUp(() async {
      ...
      when(() => mockRepository.campaignListsGet()).thenAnswer(
        (_) async => chopper.Response(http.Response('[]', 200), <ListReturn>[]),
      );
      container = ProviderContainer(
        overrides: [repositoryProvider.overrideWithValue(mockRepository)],
      );
      provider = container.read(listListProvider.notifier);
      await Future(() {});
    });
